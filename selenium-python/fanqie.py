...
class Fanqie(scrapy.Spider):
    ...
    def parse_book(self,response):
        meta=response.meta
        word_count=response.css(".info-count-word span.detail::text").get()
        read_count=response.css(".info-count-read span.detail::text").get()
        tags=response.css(".info-label-grey::text").getall()        
        total_chapters=response.css("div.volume font:nth-child(4)::text").get()    
    ...
    def parse_chp(self,response):
        meta=response.meta
        chpt1_date=response.css(".volume_first font:nth-child(4) font::text").get()
        data={
            "title": meta['book_name'],"id":meta['book_id'], "url": meta['url'], "word_count": meta['word_count'], "read_count": meta['read_count'], "chapter_count": meta['total_chapters'], "author": meta['author'], "tags": meta['tags'], "synopsis": meta['abstract'], "1st chapter date": chpt1_date
        }
