...
def main(output_dict):    
    ...
    while True:     
        ...
        for novel in novels:
            ...
            print(URL)
            df.loc[len(df.index)] = [str(bookId),str(title),str(URL),str(tags),str(words),str(totalClicks),str(monthlyClicks), str(weeklyClicks),str(finished),str(first_chapter_time),str(last_chapter)]
        params['page'] = str(int(params['page']) + 1)

    output_dict['output']=df
