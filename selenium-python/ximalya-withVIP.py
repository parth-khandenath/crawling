...
class XimalayaSpider3(scrapy.Spider):
    name = "ximalaya_3"
    ...
    def parse_album(self, response):
        ...
        for da in data["data"]["albums"]:
            ...
            author_url = f"https://www.ximalaya.com/revision/album/v1/simple?albumId={album_id}"
    ...
    def closed(self, reason):
        ...
        ans.to_csv(f"out/ximalaya_{self.book_name}.csv", index=False)
...
def main():
    ...
    def next_crawl(i):
        try:
            output_dict[names[i]] = []
            a = process.crawl(XimalayaSpider3, start_urls=links[i],
                            name=names[i], output_dict=output_dict[names[i]])
